 ...
 * Ключами об'єкту стають ключі словника,
 * а значеннями об'єкту - значення відповідних ключів у словнику.
 *
...
function setValue(dictionary, key, value) {
  ...
  //де значення буде таким же як ключ
  dictionary.forEach((values, keys) =>
    keys === key
      ? dictionary.set(keys, values.toUpperCase())
...
// Завдання: 6
/**
 * Функція `addKeyValuePairs` додає пари ключ-значення до словника
 * з перевіркою на унікальність ключа.
...
function addKeyValuePairs(dictionary, entries) {
  ...
  // Якщо ключ є унікальним, додаємо його до словника за допомогою методу
  //set та збільшимо added на 1
...
// Завдання: 7
/**
 * Функція `transformDictionary` перетворює словник в об'єкт, який містить масиви ключів,
 *  значень і пар ключ-значення.
...
function transformDictionary(dictionary) {
  // Створюємо порожні масиви для ключів (змінна keys), значень (змінна values)
  // і пар ключ-значення (змінна entries)
 ...
 * Повертаємо - Масив булевих значень: true, якщо ключ присутній у словнику,
 * та false, якщо ключ відсутній.
 */
function checkPresence(dictionary, keys) {
  ...
  // Додаємо результат перевірки (true або false) до масиву presence
  //при наявності ключа у словнику
  // Повертаємо масив
  let presence = [];
...
// Завдання: 9
/**
 * Функція `getFilteredDictionarySize` повертає кількість елементів в словнику,
 * значення яких відповідають заданому фільтру.
 * dictionary - Словник, розмір якого потрібно отримати.
 * filter - Фільтрувальна функція. Ця функція приймає пару ключ-значення та повертає true,
 * якщо пара відповідає фільтру, або false в іншому випадку.
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  // Використовуємо for...of цикл разом з методом entries() для перебору пар [ключ, значення]
  //словника
...
// Завдання: 11
/**
 * Функція `resetDictionary` перевіряє кількість елементів у словнику, 
 * та якщо вона більша ніж задана, очищує словник.
...
function resetDictionary(dictionary, maxSize) {
  ...
  // Якщо кількість елементів більша ніж максимально допустима, 
  //очищуємо словник за допомогою методу clear
  // Повертаємо true, оскільки словник був очищений
  // Повертаємо false, оскільки кількість елементів не перевищує максимально допустиму, 
  //тому словник не був очищений
 ...
 *
 * Повертає - Словник, який містить елементи множини як ключі та їх коди перших 
 * символів як значення.
 */
...
dictionary.set(value, value.charCodeAt(0));
}
return dictionary;
}
